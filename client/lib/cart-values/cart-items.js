...
function cartItemShouldReplaceCart( cartItem, cart ) {
	if (
		isRenewal( cartItem ) && ! isPrivacyProtection( cartItem ) && ! isDomainRedemption( cartItem )
	) {
	...
	return uniq(
		flatten(
			dependentProducts.concat(
				dependentProducts.map(
					dependentProduct => getDependentProducts( dependentProduct, cart ),
				),
			),
		),
	);
