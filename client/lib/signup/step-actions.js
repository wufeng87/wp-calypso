...
function createSiteOrDomain( callback, dependencies, data, reduxStore ) {
	...
	if ( designType === 'domain' ) {
		...
		SignupCart.createCart(
			cartKey,
			[ domainItem ],
			error => callback( error, providedDependencies ),
		);
			...
			( errors, providedDependencies ) => {
				callback(
					...
					pick(
						...
						[ 'siteId', 'siteSlug', 'themeSlugWithRepo', 'domainItem' ],
					),
				);
			},
			dependencies,
...
const exported = {
	...
	addPlanToCart( callback, { siteId }, { cartItem, privacyItem } ) {
		...
		SignupCart.addToCart(
			siteId,
			newCartItems,
			error => callback( error, { cartItem, privacyItem } ),
		);
	},
	...
	createAccount(
		callback,
		dependencies,
		{ userData, flowName, queryArgs, service, token },
		reduxStore,
	 ) {
		const surveyVertical = getSurveyVertical( reduxStore.getState() ).trim();
		...
		if ( service ) {
			...
			wpcom.undocumented().usersNew( assign( {}, userData, {
...
export const { addPlanToCart, createAccount, createSite } = exported;
