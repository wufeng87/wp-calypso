...
const olark = {
	...
	configureOlark: function( wpcomOlarkConfig = {}, dispatch ) {
		...
		updateDetailsEvents.forEach(
			eventName => olarkEvents.on( eventName, olarkActions.updateDetails ),
		);
	...
	updateOlarkGroupAndEligibility() {
		this.getOlarkConfiguration()
			.then( configuration => {
				const isUserEligible = 'undefined' === typeof configuration.isUserEligible
					? true
					: configuration.isUserEligible;
	...
	setOlarkOptions( userData, wpcomOlarkConfig = {} ) {
		...
		const isUserEligible = 'undefined' === typeof wpcomOlarkConfig.isUserEligible
			? true
