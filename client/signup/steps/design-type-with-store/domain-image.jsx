/**
 * External dependencies
 */
import React from 'react';

const DomainImage = () =>
	<svg width="443px" height="71px" viewBox="0 0 443 71" xmlns="http://www.w3.org/2000/svg">
		<g id="Page-1" stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
			<g id="Artboard" transform="translate(-1660.000000, -488.000000)">
				<g id="domain-only" transform="translate(1660.000000, 488.000000)">
					<g id="Rectangle-2">
						<path
							d="M0,5.99222967 C0,2.68281261 2.68574448,0 5.99992833,0
							L437.000072,0 C440.313741,0 443,2.6858357 443,5.99222967 L443,71 L0,71 L0,5.99222967 Z"
							fill="#FFFFFF"
						/>
						<path
							stroke="#D9E8F0"
							strokeWidth="2"
							d="M442,70 L1,70 L1,5.99222967 C1,3.23519406
							3.23793311,1 5.99992833,1 L437.000072,1 C439.76062,1 442,3.23728589 442,5.99222967 L442,70 Z"
						/>
					</g>
					<polyline
						id="Shape"
						fill="#CFDEE7"
						fillRule="nonzero"
						points="21 54 13 46 21 38 22.414 39.414 15.828 46 22.414 52.586"
					/>
					<polyline
						id="Shape"
						fill="#CFDEE7"
						fillRule="nonzero"
						points="37 54 45 46 37 38 35.586 39.414 42.172 46 35.586 52.586"
					/>
					<path
						d="M68.91,48 C68.432,50.833 65.967,53 63,53 C59.692,53 57,50.308 57,47
						C57,43.692 59.692,41 63,41 L65.172,41 L63.086,43.086 L64.5,44.5 L69,40
						L64.5,35.5 L63.086,36.914 L65.172,39 L63,39 C58.582,39 55,42.582 55,47 C55,51.418
						58.582,55 63,55 C67.08,55 70.438,51.945 70.93,48 L68.91,48 Z"
						id="Shape"
						fill="#CFDEE7"
						fillRule="nonzero"
					/>
					<path
						d="M0,6.00009649 C0,2.6863347 2.68574448,0 5.99992833,0 L437.000072,0 C440.313741,0
						443,2.68315617 443,6.00009649 L443,21.1854839 L0,21.1854839 L0,6.00009649 Z"
						id="Rectangle-2"
						fill="#CFDEE7"
					/>
					<g id="Rectangle-7">
						<rect
							fill="#FFFFFF"
							x="83.6597671"
							y="30.9193548"
							width="347.019989"
							height="29.7741935"
							rx="4"
						/>
						<rect
							stroke="#CFDEE7"
							strokeWidth="2"
							x="84.6597671"
							y="31.9193548"
							width="345.019989"
							height="27.7741935"
							rx="4"
						/>
					</g>
					<rect id="Rectangle-9" fill="#E4EEF4" x="92" y="39" width="150" height="13" />
				</g>
			</g>
		</g>
	</svg>;

export default DomainImage;
