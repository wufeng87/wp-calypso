...
function isPathAllowedForDomainOnlySite( path, domainName ) {
	...
	const otherPaths = [ `/checkout/${ domainName }` ],
		startsWithPaths = [ '/checkout/thank-you', `/me/purchases/${ domainName }` ];
		...
		if ( siteId ) {
			...
			const selectOnSitesChange = () => {
				...
				if ( getSite( getState(), freshSiteId ) ) {
					...
					waitingNotice = notices.info(
						i18n.translate( 'Finishing set up…' ),
						{ showDismiss: false },
					);
					dispatch( {
						type: SITES_ONCE_CHANGED,
