...
class PlanFeatures extends Component {
	render() {
		const { planProperties, isInSignupTest } = this.props;
		const tableClasses = classNames(
			'plan-features__table',
			`has-${ planProperties.length }-cols`,
		);
		const planClasses = classNames( 'plan-features', { 'plan-features--signup': isInSignupTest } );
		const planWrapperClasses = classNames( { 'plans-wrapper': isInSignupTest } );
	...
	setScrollLeft = plansWrapper => {
		const { isInSignupTest, displayJetpackPlans } = this.props;
		...
		return (
			<PlanFeaturesItem key={ index } description={ description } hideInfoPopover={ false }>
				<span className="plan-features__item-info">
					<span className="plan-features__item-title">
						{ feature.getTitle() }
					</span>
				</span>
			</PlanFeaturesItem>
