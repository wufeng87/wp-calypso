...
const Checkout = React.createClass( {
	...
	componentDidUpdate: function() {
		...
		const previousCart = this.state.previousCart, nextCart = this.props.cart;
	...
	redirectIfEmptyCart: function() {
		...
		if (
			! this.props.cart.hasLoadedFromServer || ! isEmpty( cartItems.getAll( this.props.cart ) )
		) {
			return false;
		}
		...
		if (
			transactionStepTypes.RECEIVED_WPCOM_RESPONSE === transaction.step.name &&
			isEmpty( transaction.errors )
		) {
	...
	needsDomainDetails: function() {
		const cart = this.props.cart, transaction = this.props.transaction;
