...
export default React.createClass( {
	...
	render: function() {
		var listClass, pageNumberElement;

		switch ( this.props.pageNumber ) {
			case '...':
				pageNumberElement = (
					<li className="stats-pagination__list-item stats-pagination__ellipsis" aria-hidden="true">
						…
					</li>
				);
				break;
			case '<--':
				listClass = classNames(
					'stats-pagination__list-item stats-pagination__arrow',
					'is-left',
			...
			default:
				listClass = classNames(
					'stats-pagination__list-item stats-pagination__page-number',
					{ 'is-selected': this.props.currentPage === this.props.pageNumber },
				);
				pageNumberElement = (
					<li className={ listClass } onClick={ this.clickHandler } tabIndex="0">
						{ this.numberFormat( this.props.pageNumber ) }
					</li>
				);
				break;
		}
