...
var PageList = React.createClass( {
	...
	render: function() {
		return (
			...
			>
				<Pages { ...omit( this.props, 'children' ) } />
			</PostListFetcher>
		);
	},
} );
...
var Pages = React.createClass( {
	...
	_insertTimeMarkers: function( pages ) {
		var markedPages = [], now = this.moment(), lastMarker, buildMarker;
		...
		pages.forEach( function( page ) {
			var date = this.moment( page.date ), marker = buildMarker( date );
		...
		if ( this.props.search ) {
			return (
				<NoResults
					...
					text={ this.translate( 'No pages match your search for {{searchTerm/}}.', {
						components: {
		...
		const rows = pages.map( function( page ) {
			...
			return (
				<Page
					key={ 'page-' + page.global_ID }
					page={ page }
					multisite={ this.props.siteID === false }
				/>
			);
		}, this );
	...
	render: function() {
		const { hasSites, loading, posts } = this.props;
