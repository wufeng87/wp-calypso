	...
	switch ( type ) {
		case PRIMARY_DOMAIN_CHANGE_SUCCESS:
			Object.assign( props, {
				text: (
					<span key="text">
						{ i18n.translate(
							'Primary Domain changed: All domains will redirect to' +
								' {{em}}%(domainName)s{{/em}}.',
		...
		case PRIMARY_DOMAIN_CHANGE_FAIL:
			Object.assign( props, {
				text:
					errorMessage ||
					i18n.translate( "Something went wrong and we couldn't change your Primary Domain." ),
				status: 'is-error',
				showDismiss: true,
			} );
			break;
		case PRIMARY_DOMAIN_REVERT_SUCCESS:
			Object.assign( props, {
				text: (
					<span key="text">
						{ i18n.translate(
							'No worries, your Primary Domain has been set back to' +
								' {{em}}%(domainName)s{{/em}}.',
							{ args: { domainName }, components: { em: <em /> } },
						) }
					</span>
		...
		case PRIMARY_DOMAIN_REVERT_FAIL:
			Object.assign( props, {
				text: (
					<span>
						{ errorMessage ||
							i18n.translate( "Something went wrong and we couldn't revert the changes" ) }
					</span>
				),
				status: 'is-error',
				showDismiss: true,
