...
const Unlocked = React.createClass( {
	...
	handleCancelTransferClick() {
		const { privateDomain, hasPrivacyProtection, pendingTransfer } = getSelectedDomain(
			this.props,
		);
		...
		enableDomainLocking(
			...
			error => {
				if ( error ) {
					...
					notices.success(
						this.translate(
							"We've canceled your domain transfer. Your domain is now locked and " +
					...
					notices.success(
						this.translate(
							"We've canceled your domain transfer. Your domain is now locked back.",
						),
					);
				}
	...
	render() {
		...
		if ( pendingTransfer ) {
			...
			domainStateMessage = this.translate(
				'Your domain is unlocked and Privacy Protection has been disabled' +
		...
		return (
			<div>
				<SectionHeader
					label={ this.translate( 'Transfer Domain' ) }
					className="transfer__section-header"
				>
					<Button
						...
						compact
					>
						{ this.translate( 'Cancel Transfer' ) }
					</Button>
					{ ! manualTransferRequired &&
						<Button
							...
							primary
						>
							{ this.translate( 'Resend Transfer Code' ) }
						</Button> }
				</SectionHeader>
