...
const SinglePlugin = React.createClass( {
	...
	displayHeader() {
		...
		return (
			<HeaderCake isCompact={ true } onClick={ this.goBack } onBackArrowClick={ recordEvent } />
		);
	},
		...
		return (
			<div>
				<PluginSiteList
					...
					title={ this.translate( 'Installed on', {
				...
				{ plugin.wporg &&
					<PluginSiteList
						...
						title={ this.translate( 'Available sites', {
...
export default connect(
	( state, props ) => {
		...
		const selectedSite =
			site && isJetpackSite( state, selectedSiteId )
