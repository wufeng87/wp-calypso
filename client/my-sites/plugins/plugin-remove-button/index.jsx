		...
		if ( ! this.props.site.canUpdateFiles && this.props.site.options.file_mod_disabled ) {
			...
			if ( reasons.length > 1 ) {
				html.push(
					<p key="reason-shell">
						{ this.translate(
							'%(pluginName)s cannot be removed:',
							{ args: { pluginName: this.props.plugin.name } },
						) }
					</p>,
				);
				const list = reasons.map(
					( reason, i ) => <li key={ 'reason-i' + i + '-' + this.props.site.ID }>{ reason }</li>,
				);
				html.push(
					<ul className="plugin-action__disabled-info-list" key="reason-shell-list">{ list }</ul>,
				);
			} else {
				html.push(
					<p key="reason-shell">
						{ this.translate(
							'%(pluginName)s cannot be removed. %(reason)s',
