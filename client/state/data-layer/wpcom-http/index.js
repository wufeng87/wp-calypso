...
export const queueRequest = ( processOutbound, processInbound ) => (
	{ dispatch },
	rawAction,
	next,
 ) => {
	...
	const request = fetcherMap( method )(
		...compact( [
			...
			( error, data, headers ) => {
				...
				return !! nextError
					? failures.forEach(
							handler => dispatch( extendAction( handler, failureMeta( nextError, nextHeaders ) ) ),
						)
					: successes.forEach(
							handler => dispatch( extendAction( handler, successMeta( nextData, nextHeaders ) ) ),
						);
			},
		] ),
	);
