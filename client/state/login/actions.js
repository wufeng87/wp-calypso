...
function getErrorMessageFromErrorCode( code ) {
	const errorMessages = {
		...
		push_authentication_throttled: translate(
			'You can only request a code via the WordPress mobile app once every ' +
				'two minutes. Please wait and try again.',
		),
		sms_code_throttled: translate(
			'You can only request a code via text message once per minute. Please wait and try again.',
		),
		sms_recovery_code_throttled: translate(
			'You can only request a recovery code via text message once per minute. ' +
				'Please wait and try again.',
		),
		unknown: translate(
			"Hmm, we can't find a WordPress.com account with this username and password combo. " +
...
const wpcomErrorMessages = {
	user_exists: translate(
		'Your Google email address is already in use WordPress.com. ' +
...
export const loginUser = ( usernameOrEmail, password, rememberMe, redirectTo ) => dispatch => {
	...
	return request
		.post( 'https://wordpress.com/wp-login.php?action=login-endpoint' )
		.withCredentials()
...
export const loginSocialUser = ( service, token, redirectTo ) => dispatch => {
	...
	return request
		.post( 'https://wordpress.com/wp-login.php?action=social-login-endpoint' )
		.withCredentials()
		.set( 'Content-Type', 'application/x-www-form-urlencoded' )
		.accept( 'application/json' )
		.send( {
...
export const createSocialUser = ( service, token, flowName ) => dispatch => {
	...
	return wpcom.undocumented().usersSocialNew( service, token, flowName ).then(
		wpcomResponse => {
			const data = {
...
export const sendSmsCode = () => ( dispatch, getState ) => {
	...
	return request
		.post( 'https://wordpress.com/wp-login.php?action=send-sms-code-endpoint' )
		.set( 'Content-Type', 'application/x-www-form-urlencoded' )
		.accept( 'application/json' )
		.send( {
			user_id: getTwoFactorUserId( getState() ),
