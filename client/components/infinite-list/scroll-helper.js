...
class ScrollHelper {
	...
	storeRowItemHeights( fromDirection, index ) {
		this.forEachInRow(
			index,
			function( item ) {
				const itemKey = this.props.getItemRef( item ), itemBounds = this.boundsForRef( itemKey );
				let height;
	...
	getRowHeight( index ) {
		...
		this.forEachInRow(
			index,
			item => {
				const itemKey = this.props.getItemRef( item ), height = this.storedItemHeight( itemKey );
