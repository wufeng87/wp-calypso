...
function wpcomPlugin( editor ) {
	var DOM = tinymce.DOM, each = tinymce.each, style;
	...
	editor.addCommand( 'WP_More', function( tag ) {
		...
		// Most common case
		if (
			node.nodeName === 'BODY' || ( node.nodeName === 'P' && node.parentNode.nodeName === 'BODY' )
		) {
	...
	editor.on(
		...
		function() {
			...
			function create( buttons, bottom ) {
				var toolbar, toolbarItems = [], buttonGroup;
					...
					function bindSelectorChanged() {
						...
						if ( itemName === 'bullist' ) {
							selection.selectorChanged( 'ul > li', function( state, args ) {
								var i = args.parents.length, nodeName;
