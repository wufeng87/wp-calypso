...
const HelpModal = React.createClass( {
	...
	getTableHead() {
		return (
			<thead>
				<tr>
					<th className="wpcom-help__key">
						{ this.getKeyLabel() }
					</th>
					<th className="wpcom-help__action">
						{ this.getActionLabel() }
					</th>
					<th className="wpcom-help__key">
						{ this.getKeyLabel() }
	...
	render() {
		...
		const additionalText = this.props.macosx
			? this.translate( 'Additional shortcuts, Control + Option + key:', {
					context: 'Mac shortcuts',
				} )
			: this.translate( 'Additional shortcuts, Shift + Alt + key:', {
		...
		return (
			...
			>
				<h2 className="wpcom-help__heading">
					{ this.translate( 'Keyboard Shortcuts' ) }
				</h2>
				<p>
					{ defaultText }
				</p>
