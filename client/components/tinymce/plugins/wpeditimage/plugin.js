...
function wpEditImage( editor ) {
	...
	const each = tinymce.each, iOS = tinymce.Env.iOS;

	function isPlaceholder( node ) {
		return !! ( editor.dom.getAttrib( node, 'data-mce-placeholder' ) ||
	...
	editor.on( 'init', function() {
		...
		editor.on( 'wpLoadImageData', function( event ) {
			let parent;
			const data = event.imgData.data, imgNode = event.imgData.node;
	...
	editor.on( 'BeforeExecCommand', function( event ) {
		...
		const cmd = event.command, dom = editor.dom;
