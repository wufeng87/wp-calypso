...
export const PostEditor = React.createClass( {
	...
	isPostPublishConfirmationABTest:
	...
	onSave: function( status, callback ) {
		...
		actions.saveEdited(
			edits,
			{ isConfirmationFeatureEnabled: isConfirmationFeatureEnabled },
			function( error ) {
				if ( error && 'NO_CHANGE' !== error.message ) {
					this.onSaveDraftFailure( error );
				} else {
	...
	onPublish: function( isConfirmed = false ) {
		...
		if ( isConfirmationFeatureEnabled && false === isConfirmed ) {
			this.setConfirmationSidebar( { status: 'open' } );
			return;
		}

		if ( isConfirmationFeatureEnabled && 'open' === this.state.confirmationSidebar ) {
